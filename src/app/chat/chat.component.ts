...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  public currentGroups: Array<ChatGroup>;
  ...
  constructor(private socketService: SocketService, private userHelper: UserHelper, private groupHelper: GroupHelper, private groupService: GroupService) {
    ...
    this.groupHelper.getCurrentGroupList().subscribe(gl => this.currentGroups = gl);
  }
  ...
  public send() {
    if (this.messageContent) {
      ...
      var pathToGroup:GroupPath = new GroupPath();
      pathToGroup = this.groupHelper.findPathFromRootToChild(this.currentSelectedGroup, this.currentGroups, pathToGroup);
      this.socketService.send(new SendMessage(newMessage,pathToGroup));
